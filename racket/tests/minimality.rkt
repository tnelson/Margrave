...
(start-margrave-engine #:margrave-params '("-log" "-min")
;(start-margrave-engine #:margrave-params '("-log")
                       #:margrave-path "../")
(m-load-policy "mypol1" "conference1.p")
(m-load-policy "fw1" "min-firewall.p")
...
(m-let "QFW1" '(
                ...
                ;[portdest Port]
                [pro Protocol]) 
       ;'(and ([fw1 permit] ipsrc ipdest portsrc portdest pro) (PTCP pro)))      
       '(and ([fw1 permit] ipdest pro) (PTCP pro)))      
(check-true (m-is-poss? "QFW1"))



;(m-let "Q2" '([s Subject] [a Action] [r Resource]) 
;       '([mypol1 paperassigned_applies] s a r))
