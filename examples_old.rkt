...
(display-response (mtext "EXPLORE readpaper(a) and paper(r) and not conf1:permit(s, a, r)"))
(mtext "SHOW CEILING")
...
; TODO: ~~~~~ 
; Note by Tim: so we do need to fix sort inference in order for the basic examples to work.
; deal with this tomorrow or over the weekend.
; TODO: ~~~~~
...
(mtext "EXPLORE reviewer(s) AND readpaper(a) AND paper(r)
...
(mtext "RENAME LAST notallowed")

; *** Query: There is a reviewer to whom the last query does not apply."
(mtext "EXPLORE reviewer(s) AND readpaper(a) AND NOT notallowed(s, a)")
(mtext "COUNT 0")
...
(mtext "EXPLORE reviewer(s) AND readpaper(a) AND NOT notallowed(s, a) AND paper(p)")
(mtext "COUNT 0")
...
(mtext "EXPLORE (conf1:Permit(sub, act, res) AND NOT conf2:Permit(sub, act, res)) OR
...
(mtext "EXPLORE author(s) AND last(s, a, r)")
(mtext "COUNT 0")
(mtext "get one 0")
...
(mtext "EXPLORE Phone1:TollFree(ncaller, nreceive)")
(mtext "COUNT 0")
(mtext "get one 0")
(pause-for-user)
...
(mtext "EXPLORE Firewall1:Accept(ipsrc, ipdest, portsrc, portdest, pro)")
(mtext "COUNT 0")
(mtext "get one 0")
(pause-for-user)
...
(mtext "COMPARE Firewall1 Firewall1a")
(mtext "COUNT 0")
(mtext "get one 0")
(pause-for-user)
...
; What packets are allowed through using TCP to port 21?
(mtext "EXPLORE Firewall2:Accept(ipsrc, ipdest, portsrc, portdest, pro) and tcp(pro) and port21(portdest)")
(mtext "COUNT 0")
(mtext "get one 0")
...
(mtext "EXPLORE HRLess:Accept(ipoutside, ipinside, portoutside, portinside, pro)
