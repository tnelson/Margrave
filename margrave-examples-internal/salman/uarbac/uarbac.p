...
(Policy uses uarbac
	 ...
	 (pu User))
	  ...
	  (PermViaCls = (permit u p o) :- (exists r Role (exists c Class (and (ua u r) (cpa r p c) 	  
	  	      		       	  	    	 	   	      (or (and (User o) (= c 'user))
									      	  (and (Role o) (= c 'role))
          ...
          (HierPermViaCls = (permit u p o) :- (exists subr Role (exists supr Role (exists c Class (and (ua u supr) (rh supr subr) (cpa subr p c) 
	  		    	      	      	      	   		     	  	    	       (or (and (User o) (= c 'user))
									      	  		       	   (and (Role o) (= c 'role))
										  			   (and (Course o) (= c 'course))
										  			   (and (Lab o) (= c 'lab))
	  ...
	  (DelObjViaCls = (deleteObject u o) :- (exists r Role (exists c Class (and (ua u r) (cpa r 'admin c) 
	  		  		     		       	       	       	    (or (and (User o) (= c 'user))
									      	    	(and (Role o) (= c 'role))
										  	(and (Course o) (= c 'course))
										  	(and (Lab o) (= c 'lab))
	  ...
	  (GrntObjPermToRoleViaClass = (grantObjectPermToRole u pr p o) :- (exists r1 Role (exists r2 Role (exists c Class (and (ua u r1) (ua u r2)
	  			       			      	     	   	      	   	      	   	     	   	 (or (and (User o) (= c 'user))
									      	  						     (and (Role o) (= c 'role))
										  						     (and (Course o) (= c 'course))
										  						     (and (Lab o) (= c 'lab))
										  						     (and (Department o) (= c 'department)))
																 (cpa r1 'admin c) (opa r2 'admin pr))))))
	  ...
	  (GrntObjPermToRoleViaClsAndRoleCls = (grantObjectPermToRole u pr p o) :- (exists r1 Role (exists r2 Role (exists c Class (and (ua u r1) (ua u r2)
	  				       			      	     	   	      	   	      	   	     	   	(or (and (User o) (= c 'user))
									      	  							    (and (Role o) (= c 'role))
										  							    (and (Course o) (= c 'course))
										  							    (and (Lab o) (= c 'lab))
