...
Domain-specific assertions about how sorts and predicates can be populated. Margrave will not consider scenarios that violate vocabulary constraints, ruling out non-sensical results. Valid @deftech{constraint} types are:


@subsubsection{At-most-one}

If a sort is @deftech{at-most-one} constrained, it can never contain more than one atom.

<<< what's an atom? >>>

@subsubsection{Singleton}

If a sort is @deftech{singleton} constrained, it must contain exactly one atom.
...
An @deftech{abstract} sort is fully covered by its children. For instance, consider a sort Bit with child sorts ZeroBit and OneBit. If Bit is abstract, every Bit must be either a zero or a one. 
