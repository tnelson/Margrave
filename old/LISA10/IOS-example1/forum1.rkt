...
(parse-and-load-ios "config.txt" "config-ack" "" "ack")
(parse-and-load-ios "config.txt" "config" "" "1")
(parse-and-load-ios "config-revised.txt" "config-revised" "" "2")
(parse-and-load-ios "config.txt" "config-reflexive" "" "3")
...
(m-let "Q1" 
       vardec-20
       `(and (not (IP-192.168.2.0/255.255.255.0 src-addr-in))
                  (Fastethernet0 entry-interface)
                  (Prot-TCP protocol)
                  (Port-80 src-port-in)
                  ([passes-firewall1] ,@policyvector)
                  ([internal-result1] ,@reqvector)
                  (or (Port-20 dest-port-in)))
       #:debug 3)
  
(m-count-scenarios "Q1")
